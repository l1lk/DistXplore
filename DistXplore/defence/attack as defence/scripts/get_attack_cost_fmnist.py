...
def generate_attack_cost(kmodel, x_input, y_input, dataset, attack_method, gtruth, target):
    ...
    save_path = '../results/detector/fmnist_softmax_mmd_%s_%s' % (gtruth, target)
    # save_path = '../results/detector/mnist_softmax_hda_0'
...
if __name__ == "__main__":
    ...
    # model.load_weights("/data1/wlt/vgg16_svhn_weights.h5")
    ...
    for gtruth in [args.select_truth]:
        ...
        for target in target_list:
            ...
            x_adv = np.load("/data/wlt/distribution-aware-data/defesnse crashes/fmnist/mmd/seed_v1/data_%s_%s.npy"%(gtruth, target))
            y_adv = np.load("/data/wlt/distribution-aware-data/defesnse crashes/fmnist/mmd/seed_v1/ground_truth_%s_%s.npy"%(gtruth, target))
            ...
            generate_attack_cost(model, x_adv, y_adv, 'mnist', "JSMA", gtruth, target)

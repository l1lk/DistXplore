...
class FuzzQueue(object):
    ...
    def __init__(self, outdir, is_random, sample_type, cov_num, criteria):
        ...
        if criteria == "lsa":
            # print("bi----", criteria)
            self.virgin_bits = np.zeros(1001, dtype=np.uint8)
        else:
    ...
    def has_new_bits(self, seed):
        if self.criteria == "lsa":
            temp = seed.coverage
            temp = temp.astype(np.uint8)
            cur = np.bitwise_or(self.virgin_bits, temp)
            # print(cur)
        else:
    ...
    def compute_cov(self):
        ...
        if self.criteria == "lsa":
            coverage = np.sum(self.virgin_bits) / len(self.virgin_bits)
        else:

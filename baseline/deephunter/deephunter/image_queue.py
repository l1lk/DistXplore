...
class ImageInputCorpus(FuzzQueue):
    ...
    def save_if_interesting(self, seed, data, label, crash, dry_run = False, suffix = None):
        ...
        if crash:
            fn = "%s/crashes/id_%06d_%s.npy" % (self.out_dir, self.uniq_crashes, describe_op)
            fn_label = "%s/crash_labels/id_%06d_%s" % (self.out_dir, self.uniq_crashes, describe_op)
            fn_seed = "%s/crash_seeds/id_%06d_%s" % (self.out_dir, self.uniq_crashes, describe_op)
            self.uniq_crashes += 1
            self.last_crash_time = current_time
        else:

            fn = "%s/queue/id_%06d_%s.npy" % (self.out_dir, self.total_queue, describe_op)
            fn_label = "%s/queue_labels/id_%06d_%s" % (self.out_dir, self.total_queue, describe_op)
            fn_seed = "%s/queue_seeds/id_%06d_%s" % (self.out_dir, self.total_queue, describe_op)
            ...
            if self.has_new_bits(seed) or dry_run :
                ...
                del seed
                return False
        # print("____________", np.unique(data))
        np.save(fn, data)
        np.save(fn_label, label)
        np.save(fn_seed, seed.root_seed)
        return True
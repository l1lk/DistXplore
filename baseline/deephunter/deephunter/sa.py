...
def fetch_lsa(model, x_train, x_target, target_name, layer_names, args):
    ...
    # print( "Fetching LSA")
    if args.is_classification:
        # print(len(target_ats))
        for i, at in enumerate(target_ats):
            label = target_pred[i]
            kde = kdes[label]
            if i==0:
                lsa=_get_lsa(kde, at, removed_cols)
                i=i+1
            else:
                lsa=np.append(lsa,_get_lsa(kde, at, removed_cols))
...
def get_sc(lower, upper, k, sa, ptr):
    """Surprise Coverage
    ...
    """
    sa = (np.array(sa)/10000).tolist()
    if len(np.array([sa]).reshape(-1))==1:
        sa = np.array([sa])
    for idx, tmp_sa in enumerate(sa):
        buckets = np.digitize(sa, np.linspace(lower, upper, k))
    # print("tests", buckets)
    # print(len(np.array([buckets]).reshape(-1))==1)
        bucket_list=list(set(buckets))
    # ptr=np.zeros(k+1)
    # print("testtest", ptr.shape)
        for i in bucket_list:
            ptr[idx][i]=1
    # ptr=ptr.reshape(1,ptr.shape[0])
    # print("tttest", ptr.shape)

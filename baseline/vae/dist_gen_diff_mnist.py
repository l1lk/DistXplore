...
from Vae_FMMNIST_NN1 import Vae_FMMNIST_NN1
from utils import *
# from utils_svhn import *
import imageio
import numpy as np
import math
import time
import datetime
from vgg16_svhn import svhnvgg
from vgg16_cifar10 import cifar10vgg
import tqdm
random.seed(3)
from get_model import mnist_lenet4

def to_image(x):
    x = np.uint8(x)
    x = np.reshape(x, (28, 28)) # shape[0], shape[1]
...
vae = Vae_MNIST_NN1(input_tensor=model.input)
...
seeds = np.load("single_cluster_seeds/mnist/training_100/class_%s_seed.npy"%seed_index)
...
seeds = mnist_preprocessing(seeds)
...
for current_seed in tqdm.tqdm(seeds):
    ...
    if label != truth and not isInvalid(gen_img, vae, vae_threshold):
        ...
        gen_img_deprocessed = to_image(gen_img)
        ...
        imageio.imwrite(output_directory + 'already_differ_' + str(current_seed) + '_' + str(truth) + '_' + str(label) + '.png', gen_img_deprocessed)
        continue
    ...
    iterate = K.function([model.input], [loss, loss_neuron, grads])
    ...
    for iters in range(args.grad_iterations):
        ...
        loss_value, loss_neuron, grads_value = iterate([gen_img]) 
        # print(4.1)
        ...
        if predictions != truth:
            ...
            # print(4.8)
            gen_img_deprocessed = to_image(gen_img)
            orig_img_deprocessed = to_image(orig_img)
            imageio.imwrite(
                    output_directory + str(loop_index) + '_' + str(
